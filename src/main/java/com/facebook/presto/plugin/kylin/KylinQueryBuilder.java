...
public class KylinQueryBuilder
        extends QueryBuilder
{
    ...
    private static class KylinTypeAndValue
    {
        private final Type type;
        private final Object value;

        public KylinTypeAndValue(Type type, Object value)
        {
    ...
    public KylinQueryBuilder(String quote)
    {
        super(quote);
...
//        }
        ...
        List<KylinTypeAndValue> accumulator = new ArrayList<>();
        ...
        for (int i = 0; i < accumulator.size(); i++) {
            KylinTypeAndValue typeAndValue = accumulator.get(i);
    ...
    private List<String> toConjuncts(List<JdbcColumnHandle> columns, TupleDomain<ColumnHandle> tupleDomain, List<KylinQueryBuilder.KylinTypeAndValue> accumulator)
    {
    ...
    private String toPredicate(String columnName, Domain domain, Type type, List<KylinQueryBuilder.KylinTypeAndValue> accumulator)
    {
    ...
    private String toPredicate(String columnName, String operator, Object value, Type type, List<KylinQueryBuilder.KylinTypeAndValue> accumulator)
    {
        bindValue(value, type, accumulator);
    ...
    private static void bindValue(Object value, Type type, List<KylinQueryBuilder.KylinTypeAndValue> accumulator)
    {
        checkArgument(isAcceptedType(type), "Can't handle type: %s", type);
        accumulator.add(new KylinQueryBuilder.KylinTypeAndValue(type, value));
    }
}